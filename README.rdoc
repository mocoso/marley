...
I simply brough +config.ru+ and +marley.rb+ out of their folders so Heroku can recognize this as a sinatra app. Additionally, I created a +config/database.yml+ file and told Marley to look there for database settings. I also modified the default 'data' folder settings to create it within the current directory. 
